...
{
    public class SingleRelationPatternBuilder: IPatternBuilder
    {
        private static Dictionary<string, DeepRelationName> relations;

        static SingleRelationPatternBuilder()
        {
        ...
        public Pattern Build(Node node)
        {
            ...
            var match = relations[node.Descendants.First((n) =>
                n.Label == PatternGrammar.MatchDep.Name).Text];
            return new SingleRelationPattern(name, match, save);
        }
    }
}
